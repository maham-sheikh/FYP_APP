...
const API_BASE_URL = "http://192.168.18.244:8000/api/customer";
...
function Service5details({ route, navigation }) {
  ...
  const fetchVendorsData = async () => {
    try {
      ...
      setError(error.message || "Failed to fetch healthcare providers");
    } finally {
      setLoading(false);
    }
  };
    ...
    if (vendors.length === 0) {
      return (
        <Text style={styles.noServicesText}>
          No {subService.toLowerCase()} found within {selectedDistance}
        </Text>
      );
    }
    ...
    return vendors.map((vendor, index) => {
      ...
      return (
        <View key={vendor.id} style={styles.serviceContainer}>
          ...
          <Text style={styles.serviceWork}>
            {vendor.service_type || subService}
          </Text>
  ...
  const renderButtons = () => {
    ...
    return buttons.map((button) => (
      <TouchableOpacity
        ...
        style={[
          styles.button,
          selectedDistance === button && styles.selectedButton,
        ]}
