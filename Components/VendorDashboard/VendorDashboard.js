...
function VendorDashboard() {
  ...
  const handleRequestAction = async (requestId, newStatus) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/request-action`, {
        requestId,
        action: newStatus
      });
          ...
          requests.map(request => (
            <View key={request.id} style={styles.requestCard}>
              <View style={styles.cardHeader}>
                <Text style={styles.customerName}>{request.customer_name || 'Unknown Customer'}</Text>
                <Text style={styles.timeAgo}>{formatTimeAgo(request.created_at)}</Text>
              </View>
              
              <View style={styles.contactSection}>
                <Text style={styles.sectionTitle}>Contact</Text>
                <Text style={styles.contactText}>{request.customer_phone || 'Phone not available'}</Text>
              </View>
